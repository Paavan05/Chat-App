...
export const ChatContainer = () => {
    ...
    return selectedUser ? (
        <div className='h-full flex flex-col overflow-hidden backdrop-blur-lg bg-white/10 dark:bg-[#080809] pl-2'>
            <div className='flex items-center gap-3 py-3 mx-1 ml-0 border-b border-stone-500 shrink-0'>
            ...
            {/* masseage container */}
            <div className='flex-1 overflow-y-auto pt-3 pr-2 pb-6 '>
                    ...
                    <div key={index} className={`flex items-end gap-2 justify-end  ${msg.senderId !== authUser._id && 'flex-row-reverse'} `}>
                        {msg.image ? (
                            <div className='flex relative mb-2 '>
                                ...
                                <p className={`absolute bottom-2.5  text-[10px] text-white bg-[#0b141a]/70 px-1.5 py-0.5 rounded-md ${msg.senderId === authUser._id ? 'right-1': 'left-1'} `}>
                                    {formatMassageTime(msg.createdAt)}
                                </p>
                            </div>
                        ) : (
                            <div className={`flex relative p-2 h-12 max-w-[200px] min-w-[80px] md:text-sm font-light rounded-1g mb-6 rounded-2xl break-all bg-blue-400 dark:bg-gray-300 text-black ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>
                                <p className=''>{msg.text}</p>
                                <p className={`absolute bottom-0 text-[10px] text-black px-1.5 ${msg.senderId === authUser._id ? 'right-1': 'left-1'} `}>
                                    {formatMassageTime(msg.createdAt)}
                                </p>
                            </div>
                        )}
        ...
        <div className='flex flex-col justify-center items-center h-full gap-2 text-gray-500 bg-white/10 dark:bg-[#080809] max-md:hidden rounded-r-xl'>
